...
import {
  Plus, Search, Phone, MessageCircle, Monitor, Globe, CheckCircle2, XCircle,
} from 'lucide-react'
...
export default function AppointmentsPage() {
  ...
  async function handleNewAppointment() {
    ...
    const { data: existing } = await supabase
      .from('customers').select('id').eq('business_id', businessId).eq('phone', form.customer_phone).single()
    ...
    if (customerId) {
      ...
      const { error } = await supabase.from('appointments').insert({
        business_id: businessId, barber_id: form.barber_id, customer_id: customerId,
        service_id: form.service_id, scheduled_at: form.scheduled_at,
        duration_minutes: service?.duration_minutes || 30,
        status: 'confirmed', source: 'manual', notes: form.notes,
      })

      if (error) { console.error('Error al crear cita:', error.message); setSaving(false); return }
  ...
  const filtered = appointments.filter(a => {
    ...
    return (!search || name.includes(search.toLowerCase())) && (!filterStatus || a.status === filterStatus)
  })
    ...
    <div className="mx-auto max-w-7xl p-6 lg:p-8">
      ...
      <div className="glass mb-6 rounded-2xl p-4">
        <div className="grid gap-3 lg:grid-cols-[1.4fr_220px_220px_180px]">
          ...
          <input type="date" value={filterDate} onChange={e => setFilterDate(e.target.value)} disabled={showAllDates}
            className={`rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-white outline-none focus:border-orange-500/50 ${showAllDates ? 'cursor-not-allowed opacity-50' : ''}`} />
          <div className="relative">
            ...
            <svg className="pointer-events-none absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
              <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
            </svg>
          </div>
          <button type="button" onClick={() => setShowAllDates(prev => !prev)}
            className={`rounded-2xl border px-4 py-3 text-sm font-medium transition-all ${showAllDates ? 'border-orange-500/30 bg-orange-500/10 text-orange-400' : 'border-white/10 bg-white/5 text-gray-300 hover:bg-white/8'}`}>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/5">
                <th className="px-6 py-3 text-left text-xs uppercase tracking-wider text-gray-500">{showAllDates ? 'Fecha y hora' : 'Hora'}</th>
                <th className="px-6 py-3 text-left text-xs uppercase tracking-wider text-gray-500">Cliente</th>
                <th className="hidden px-6 py-3 text-left text-xs uppercase tracking-wider text-gray-500 md:table-cell">Barbero</th>
                <th className="hidden px-6 py-3 text-left text-xs uppercase tracking-wider text-gray-500 lg:table-cell">Servicio</th>
              ...
              {loading ? (
                ...
                return (
                  <tr key={apt.id} className="transition-colors hover:bg-white/[0.02]">
                    <td className="px-6 py-4 font-syne font-bold text-white">
                      {showAllDates ? (
                        <div>
                          ...
                          <p className="text-xs font-normal text-gray-500">{format(new Date(apt.scheduled_at), 'HH:mm')}</p>
                        </div>
                    ...
                    <td className="hidden px-6 py-4 text-sm text-gray-300 md:table-cell">{(apt.barber as any)?.name}</td>
                    <td className="hidden px-6 py-4 text-sm text-gray-300 lg:table-cell">{(apt.service as any)?.name}</td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex gap-1">
                        {apt.status !== 'completed' && apt.status !== 'cancelled' && (
                          <>
                            <button onClick={() => updateStatus(apt.id, 'completed')} className="rounded-lg p-1.5 text-gray-500 transition-colors hover:bg-emerald-500/10 hover:text-emerald-400" title="Completar">
                              <CheckCircle2 className="h-4 w-4" />
                            </button>
                            <button onClick={() => updateStatus(apt.id, 'cancelled')} className="rounded-lg p-1.5 text-gray-500 transition-colors hover:bg-red-500/10 hover:text-red-400" title="Cancelar">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="glass animate-fade-up relative w-full max-w-md rounded-2xl p-6">
            ...
            <div className="mt-6 flex gap-3">
              <button onClick={() => setShowModal(false)} className="flex-1 rounded-xl border border-white/10 px-4 py-2.5 text-sm text-gray-400 transition-colors hover:text-white">Cancelar</button>
