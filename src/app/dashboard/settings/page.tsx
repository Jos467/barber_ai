  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      const { data: biz } = await supabase
        .from('businesses').select('*').eq('owner_id', data.user.id).single()
      if (!biz) return
    ...
    <div className="p-6 lg:p-8 max-w-3xl mx-auto">
          ...
          <div className="bg-white/3 rounded-xl p-4 mb-5 border border-white/8 animate-fade-up">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-3 py-2 rounded-xl border border-white/10 text-gray-400 text-xs hover:text-white transition-colors"
              >
                Cancelar
              </button>
              <button
                onClick={addService}
                disabled={!svcForm.name || !svcForm.price}
      ...
      <div className="glass rounded-2xl p-6 border border-orange-500/20">
        ...
        <p className="text-gray-400 text-sm mb-3">
          Usa estos datos para configurar n8n y Vapi en los Días 2 y 3:
        </p>
        <div className="bg-black/30 rounded-xl p-4 font-mono text-xs space-y-2">
          ...
          <p className="text-gray-400">
            API Key: <span className="text-orange-400">barberai_secret_2026</span>
          </p>
          <p className="text-gray-400">
            Header: <span className="text-blue-400">x-api-key</span>
          </p>
        </div>
      </div>
    </div>
